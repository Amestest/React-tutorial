...
function TodoList() {
    ...
    const addTodo = () => {
        //todos기본값으로 아무런 리스트가 없는 빈 배열이 존재
        //setTOdos로 기본으로 아무런 리스트가 없는 목록에 새로운 목록 하나 씩 추가하는 것을 진행
        //  [...todos] = 기존에 있떤 리스트를 복사해서
    ...
    const toggleTodo = (index) => {
      ...
      // newTodos[index].completed = !newTodos[index].completed;
      // 새로운목록들[사용자가선택한목록번호].완료/미완료 = 완료->미오나료, 미완료->완료로 상태 변경
      // !를 이용해서 boolean(true / false) 값을 (false/true) 값으로 변경할 수 잇음
    ...
    return (
      <div className="TodoList">
        ...
        {/* 할 일이 추가되면 추가된 목록을 system.out.println 와 같은 map을 이용해서
        사용자 눈에 추가도니 목록을 보여주기
        각 목록은
  ...
  /*
  추후 useEffect 사용해서 웹사이트를 새로고침(F5) 하더라도 목록이 남아있으 수 있도록 업데이트!
