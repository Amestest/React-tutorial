const User = ({ user, userList, setUserList, setEdditUser }) => {
    /*
    1번과 2번 동일
  1번 : const User = ({ user, userList, setUserList, setEdditUser }) => {
  
  2번 : const User = (props) => {
      const user = props.user;
      const userList = props.userList;
      const setUserList = props.setUserList;
      const setEdditUser = props.setEditUser;
    */  
    const deleteUser = () => {
    ...
    return (
      <tr>
        ...
        <td>
            <button onClick={() => setEdditUser(user)}>수정하기</button>
