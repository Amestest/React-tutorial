...
const Game = () => {
    ...
    useEffect(() => {
        ...
        //컴퓨터.환경에서.public으로 시작하는 폴더에 player.png 라는 이미지가 있따면 주소를 가져와서 보여주겠다.
        playerImg.onload = () => {
            playerImageRef.current = playerImg;// 개발자 전용으로 이미지를 무사히 가져왔는지 확인하는 코드
            console.log('Player 이미지 가져오기 성공');
        };
        playerImg.onerror = () => { //플레이어 이미지를 가져오지 못하면 실패를 보여줌
        ...
        const handleKeyDown = (event) => {
            ...
            switch (event.key) {
                case 'ArrowUp' : case 'w':// ↑ 키보드 모양이나 w를 누르면 위로 이동
                    ...
                    break;
                case 'ArrowDown': case 's': // ↓ 키보드 모양이나 w를 누르면 위로 이동
                    newPlayer.y = Math.min(newPlayer.y + 40, canvasRef.current.height - 40);
                    break;
                case 'ArrowLeft': case 'a':// ← 키보드 모양이나 w를 누르면 위로 이동
                    newPlayer.x = Math.max(newPlayer.x - 40, 0);
                    break;
                case 'ArrowRight': case 'd':// → 키보드 모양이나 w를 누르면 위로 이동
    ...
    useEffect(() => {
        ...
        const updateGame = () => {
            ...
            if (playerImageRef.current) { //public에 이미지가 조냊하면 이미지 보여주기
    ...
    return (
        <div className="game-container">
            {/* canvas는 동영상 캡쳐나 사진을 찍고 찍은 사진을 보여줄때 사진이 보여질 수 있는 틀을 제공
                이 코드에서는 게임 틀을 제공
            */}
