...
const Ajax_Fetch = () => {
    return(
        ...
        <pre>
            ...
            React에서는 Ajax대시 Fetch API 또는 Axios와 같은 라이브러리를 사용
            ...
            Fetch API  : 리액트에 내장되어 있는 API를 가져와서 사용
                ...
                ex) 게시판에서 게시물이 안보여요. API가 잘못된 것 같아요.
                    (속 뜻 : 백엔드에서 데이터 가져오느 주소가 잘못됐거나 그 안에 코드가 이상하다.)     
        ...
        <pre>
           Fetch ('api주소작성')  // 자바에서 try와 같은데 불러올 주소가 적힌 try
              ...
              .then(데이터 => {   // try 안에 적힌 if 문으로 만약에 어떤 데이터를 가져오는데 성공했다면
                실행할 코드 작성; //조건에 맞는 데이터가 들어왓을 때 실행할 코드를 작성
              }) 
              // 만약에 데이터 가져오는데 실패했다면
              // 에러 출력할 변수로 e 를 사용
              .catch( e => {  // 자바에서 cathc 문 또는 else 로 에러가 발생했을 때 보여줄 에러 출력
                실행할 코드 작성;
              })
        </pre>
        </>
