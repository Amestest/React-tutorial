...
const Pagination= ({itemPerPage, totalItems, paginate, currentPage}) =>{
    const 페이지번호들 = []; 
    const totalPage = Math.ceil(totalItems / itemPerPage);// 전체 페이지수 계산하고 올림처리
    ...
    const renderPageNumber= () => {
        ...
        // 현재 페이지를 중심으로 해서 앞쪽에 잇는 5개의 페이지번호를 표시할 수 있도록 설정
        // 시작페이지 번호가 -가 나올 것을 방지 Math.max
        // Math.max(1, 현재페이지가 3 - 5 = -2) 시작페이지번호는 무조건 1
        // Math.max(1, 현재페이지가8 -5 = 3) 시작페이지는  무조건 3
        // Math.max(max가 되는 숫자 작성 문지기처럼 이밑으로는 max값을 못가져오도록 설정, 숫자비교값)
        const startPage = Math.max(1, currentPage - 5);
        // 끝 페이지 번호
        // totalPage = 가장 큰 수
        // 끝 페이지 번호는 전체 페이지 수를 초과할 수 없기 때문에
        // 전체페이지 수인 totalPage가 문지기철머 이 이상으로 min값을 못 가져오도록 설정
        ...
        // startPage = 1 
        // 배열의 인덱스는 0
        // 인덱스값을 맞추기 위해 1-1 = 0 ~ 마지막 페이지
        // 예를 들어 시작페이지 3 마지막 페이지가 12  => 2, 12가 돼서 번호 3 ~ 12까지
